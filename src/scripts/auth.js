...
function scheduleRefresh(auth) {
  ...
  const rt = auth.refresh_token; if (!rt) return;
  refreshTimerId = setTimeout(async () => {
    try { await refreshAccessToken(rt); } catch (e) { console.warn('Token refresh failed:', e); }
  }, delay);
}
...
function toBase64Url(uint8Arr) {
  let str = ''; for (let i = 0; i < uint8Arr.byteLength; i++) str += String.fromCharCode(uint8Arr[i]);
  return btoa(str).replace(/\+/g, '-').replace(/\//g, '_').replace(/=+$/, '');
}
async function sha256Base64Url(input) {
  const enc = new TextEncoder(); const data = enc.encode(input);
  const digest = await crypto.subtle.digest('SHA-256', data);
  return toBase64Url(new Uint8Array(digest));
}
function generateCodeVerifier() { const b = new Uint8Array(64); crypto.getRandomValues(b); return toBase64Url(b); }
function buildAuthUrl({ state, codeChallenge }) {
  const params = new URLSearchParams({
    client_id: CLIENT_ID, response_type: 'code', redirect_uri: REDIRECT_URI,
    scope: SCOPES.join(' '), state, code_challenge_method: 'S256', code_challenge: codeChallenge
  });
...
async function exchangeCodeForTokens(code, verifier) {
  ...
  return {
    access_token: json.access_token, token_type: json.token_type, scope: json.scope,
    refresh_token: json.refresh_token || getStoredAuth()?.refresh_token || null, expires_at
  };
}
async function refreshAccessToken(refresh_token) {
  ...
  if (!res.ok) { const text = await res.text().catch(() => ''); throw new Error(`Token refresh failed: ${res.status} ${text}`); }
  const json = await res.json();
  const current = getStoredAuth() || {};
  const newRefresh = json.refresh_token || current.refresh_token || refresh_token;
  const expires_at = Date.now() + json.expires_in * 1000 - 10_000;
  setStoredAuth({ ...current, access_token: json.access_token, token_type: json.token_type || current.token_type || 'Bearer', scope: json.scope || current.scope, refresh_token: newRefresh, expires_at });
...
export async function maybeHandleRedirectCallback() {
  ...
  if (error) { console.warn('Spotify auth error:', error); return false; }
  if (code) {
    try {
      ...
      return true;
    } catch (e) { console.warn('Spotify auth exchange error:', e.message || e); return false; }
  }
  const existing = getStoredAuth(); if (existing) scheduleRefresh(existing);
  return false;
}
export async function fetchUserProfile() {
  const token = getAccessToken(); if (!token) return null;
  try {
