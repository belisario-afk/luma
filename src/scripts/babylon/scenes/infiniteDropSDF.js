// Infinite Drop SDF Tunnel — raymarched tunnel with reactive drop speed & pulses.
...
export function buildInfiniteDropSDF(scene) {
  ...
  const frag = `
    ...
    float map(in vec3 p, out float matId){
      ...
      float tunnel = length(q.xy) - radius;
      ...
      float d = min(tunnel, max(-0.04, ribs));

      vec3 stud = q;
      stud.xy = normalize(q.xy) * (radius - 0.02) + (q.xy - normalize(q.xy)*radius);
      float studs = sdBox(stud, vec3(0.02,0.02,0.2));
      d = min(d, studs);
    ...
    void main(){
      ...
      vec3 rd = normalize(vec3(uv * rot(roll), 1.6));
